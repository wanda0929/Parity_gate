...
@testset "evolve_eff" begin
    ...
    apply!(reg, put(1=>H))
    apply!(reg, put(2=>H))
    state000, state001, state110, state111 = evolve_eff(reg, Ω_f, τ, α, Ω_c, Δ, J_12, J_dd)
    @test state000 isa Vector{Float64}
    @test state001 isa Vector{Float64}
    @test state110 isa Vector{Float64}
    @test state111 isa Vector{Float64}
end
...
@testset "paritygate" begin
    ...
    print_table(reg)
    state000, state001, state110, state111, state010, state011, state100, state101 = evolve_eff(reg, Ω_f, τ, α, Ω_c, Δ, J_12, J_dd)
    times = 0:τ/Nt:τ-τ/Nt
